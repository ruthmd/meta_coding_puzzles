...
-- How do we rank by the total number of units sold?
 ...
 customers acrojoss its different stores. It also conducts many different
 promotion campaigns.
 ...
 */
 ...
 SELECT
    ...
    ROUND(
        SUM(CASE WHEN s.promotion_id IS NOT NULL THEN s.units_sold ELSE 0 END) * 1.0 /
        NULLIF(SUM(CASE WHEN s.promotion_id IS NULL THEN s.units_sold ELSE 0 END), 0),
        10
